    ...
    const val CONTENT = """
            window.initSpotilolPlayer=function(){
                ...
                pl.addEventListener('touchstart',function(e){
                    if(e.target.closest('#spl-bar')||e.target.closest('#spl-edgebar')||e.target.closest('.spl-vol-bar'))return;
                    var t=e.touches[0];
                    splDragStart(t.clientX,t.clientY);
                },{passive:true});
                ...
                pl.addEventListener('touchmove',function(e){
                    ...
                    var t=e.touches[0];
                    splDragMove(t.clientX,t.clientY);
                },{passive:false});
                ...
                pl.addEventListener('touchcancel',function(){
                    splDragEnd();
                });
                
                pl.addEventListener('mousedown',function(e){
                    if(e.button!==0)return;
                    if(e.target.closest('#spl-bar')||e.target.closest('#spl-edgebar')||e.target.closest('.spl-vol-bar')||e.target.closest('button'))return;
                    splDragStart(e.clientX,e.clientY);
                });
                
                document.addEventListener('mousemove',function(e){
                    splDragMove(e.clientX,e.clientY);
                });
